...
const mutations = {
    ...
    SET_ROLESTEXT: (state, rolesText) => {
      state.rolesText = rolesText;
    }
};
const actions = {
    // 用户登录
    login({ commit }, userInfo) {
        const { username, password, imgCode, imgCodeToken } = userInfo;
        return new Promise((resolve, reject) => {
          login(serverLoginUrl, { username: username, password: password, imgCode: imgCode, imgCodeToken: imgCodeToken }).then(response => {
            const { data } = response;
            const { token } = data;
    ...
    getInfo({ commit, state }) {
        return new Promise((resolve, reject) => {
            getInfo(state.token).then(response => {
                ...
                // data.resources = ['admin'];

                const auth = data.resources || [];
                data.resources = data.roles || [];
                data.roles = auth;

                const { roles, resources, name, avatar, msgtotal } = data;
                ...
                commit('SET_ROLESTEXT', resources);
