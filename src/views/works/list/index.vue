<script lang="ts" setup>
    ...
    const handleDelete = async (index:number, row: TableListItem): Promise<void> => {
        ...
        try {
            ...
            ElMessage.success(t('views.works.list.table.column.action.buttion.del.success'));
            /**
             * 为了减少请求，
...
<template>
    ...
    >
        ...
        <template #header>
            <el-row>
                    <el-col :span="8">
                        <router-link to="/works/add" >
                            <el-button type="primary"> {{t('views.works.list.button.add')}}</el-button>
                        </router-link>
                    </el-col>
                    <el-col :span="16" class="text-align-right">                        
                        <el-input style="width:200px;" :placeholder="t('views.works.list.search.input.placeholder')" v-model="miniSearchInput"  @keydown.enter="miniSearchBtn">
                            <template #suffix>
                                <i class="el-input__icon"><icon-svg type="search" class="cursor-pointer" @click="miniSearchBtn"></icon-svg></i>
                            </template>
                        </el-input>
                        <el-button style="margin-left: 8px" @click="() => searchDrawer.visible = true">{{t('views.works.list.button.search')}}</el-button>
                    </el-col>
            </el-row>
        </template>
        ...
        <el-table-column
            fixed
            type="index"
            :label="t('views.works.list.table.column.index')"
            width="80">
        </el-table-column>

        <el-table-column
            fixed
            :label="t('views.works.list.table.column.title')"
            prop="title"
            min-width="330">
        </el-table-column>
        ...
        <el-table-column
            prop="addtime"
            :label="t('views.works.list.table.column.addtime')"
            width="140">
        </el-table-column>
        <el-table-column
            :label="t('views.works.list.table.column.tags')"
            width="130">
        ...
        <el-table-column
            :label="t('views.works.list.table.column.action')"
            prop="action"
            width="120">
            <template #default="{row,$index}">
                <router-link :to="`/works/edit?id=${row.id}`" class="el-button el-button--text el-button--small text-decoration-none">{{t('views.works.list.table.column.action.buttion.edit')}}</router-link>
                <el-popconfirm :title="t('views.works.list.table.column.action.buttion.del.popconfirm')" @confirm="handleDelete($index, row)">
                    <template #reference>
                        <el-button type="text" :loading="row.delLoading" >{{t('views.works.list.table.column.action.buttion.del')}}</el-button>  
                    </template>
                </el-popconfirm>                       
            </template>
        </el-table-column>
     ...
     <search-drawer
        ...
        :title="t('views.works.list.components.search.title')"
