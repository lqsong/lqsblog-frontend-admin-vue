<script lang="ts" setup>
    ...
    const handleDelete = async (index:number, row: TableListItem): Promise<void> => {
        ...
        try {
            ...
            ElMessage.success(t('views.links.list.table.column.action.buttion.del.success'));
            /**
             * 为了减少请求，
    ...
    const formDialog = reactive<FormDialogProps>({
        ...
        onSubmit: async (values) => {
            if(formDialog.values) { // 编辑
                ...
                pushQuery({refresh: new Date().getTime()})
                ElMessage({
                  message: t('views.links.list.components.formdialog.editsuccess'),
                  type: 'success',
                  onClose: () => { }
                })
            } else { // 新增
                ...
                ElMessage({
                  message: t('views.links.list.components.formdialog.addsuccess'),
                  type: 'success',
                  onClose: () => {}
                })
            }    
...
<template>
    ...
    >
        ...
        <template #header>
            <el-row>
                    <el-col :span="8">
                        ...
                        <el-button type="primary" @click="addFormShow"> {{t('views.links.list.button.add')}}</el-button>
                        
                    </el-col>
                    <el-col :span="16" class="text-align-right">                        
                        <el-input style="width:200px;" :placeholder="t('views.links.list.search.input.placeholder')" v-model="miniSearchInput"  @keydown.enter="miniSearchBtn">
                            <template #suffix>
                                <i class="el-input__icon"><icon-svg type="search" class="cursor-pointer" @click="miniSearchBtn"></icon-svg></i>
                            </template>
                        </el-input>
                        <el-button style="margin-left: 8px" @click="() => searchDrawer.visible = true">{{t('views.links.list.button.search')}}</el-button>
                    </el-col>
            </el-row>
        </template>
        ...
        <el-table-column
            fixed
            type="index"
            :label="t('views.links.list.table.column.index')"
            width="80">
        </el-table-column>

        <el-table-column
            fixed
            :label="t('views.links.list.table.column.title')"
            prop="title"
            min-width="330">
        </el-table-column>
        <el-table-column
            :label="t('views.links.list.table.column.category')"
            min-width="100">
        ...
        <el-table-column
            :label="t('views.links.list.table.column.action')"
            prop="action"
            width="120">
            <template #default="{row,$index}">
                <el-button type="text" :loading="row.detailLoading" @click="editFormShow($index, row)">{{t('views.links.list.table.column.action.buttion.edit')}}</el-button> 
                
                <el-popconfirm :title="t('views.links.list.table.column.action.buttion.del.popconfirm')" @confirm="handleDelete($index, row)">
                    <template #reference>
                        <el-button type="text" :loading="row.delLoading" >{{t('views.links.list.table.column.action.buttion.del')}}</el-button>  
                    </template>
                </el-popconfirm>                       
            </template>
        </el-table-column>
