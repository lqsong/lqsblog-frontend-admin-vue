<script lang="ts" setup>
    ...
    const handleDelete = async (index:number, row: TableListItem): Promise<void> => {
        ...
        try {
            ...
            ElMessage.success(t('views.setting.accountlist.table.column.action.buttion.del.success'));
            /**
             * 为了减少请求，
    ...
    const formDialog = reactive<FormDialogProps>({
        ...
        onSubmit: async (values) => {
            if(formDialog.values) { // 编辑
                ...
                pushQuery({refresh: new Date().getTime()})
                ElMessage({
                  message: t('views.setting.accountlist.components.formdialog.editsuccess'),
                  type: 'success',
                  onClose: () => { }
                })
            } else { // 新增
                ...
                ElMessage({
                  message: t('views.setting.accountlist.components.formdialog.addsuccess'),
                  type: 'success',
                  onClose: () => {}
                })
            }    
...
<template>
    ...
    >
        ...
        <template #header>
            <el-row>
                    <el-col :span="8">
                        ...
                        <el-button type="primary" @click="addFormShow"> {{t('views.setting.accountlist.button.add')}}</el-button>
                        
                    </el-col>
                    <el-col :span="16" class="text-align-right">                        
                        <el-input style="width:200px;" :placeholder="t('views.setting.accountlist.search.input.placeholder')" v-model="miniSearchInput"  @keydown.enter="miniSearchBtn">
                            <template #suffix>
        ...
        <el-table-column
            fixed
            type="index"
            :label="t('views.setting.accountlist.table.column.index')"
            width="80">
        </el-table-column>

        <el-table-column
            fixed
            :label="t('views.setting.accountlist.table.column.nickname')"
            prop="nickname"
            min-width="330">
        </el-table-column>
        <el-table-column
            fixed
            :label="t('views.setting.accountlist.table.column.username')"
            prop="username"
            min-width="330">
        </el-table-column>
        <el-table-column
            fixed
            :label="t('views.setting.accountlist.table.column.roles')"
            prop="roles"
            min-width="330">
        ...
        <el-table-column
            :label="t('views.setting.accountlist.table.column.action')"
            prop="action"
            width="120">
            <template #default="{row,$index}">
                <el-button type="text" @click="editFormShow(row)">{{t('views.setting.accountlist.table.column.action.buttion.edit')}}</el-button> 
                
                <el-popconfirm :title="t('views.setting.accountlist.table.column.action.buttion.del.popconfirm')" @confirm="handleDelete($index, row)">
                    <template #reference>
                        <el-button type="text" :loading="row.delLoading" >{{t('views.setting.accountlist.table.column.action.buttion.del')}}</el-button>  
                    </template>
                </el-popconfirm>                       
            </template>
        </el-table-column>
    ...
    <form-dialog
        ...
        ref="formDialogRef"
        :title="formDialog.values?t('views.setting.accountlist.components.formdialog.edit.title'):t('views.setting.accountlist.components.formdialog.add.title')"
